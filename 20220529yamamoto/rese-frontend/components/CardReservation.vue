<template>
  <div class="card-reservation">
    ...
    <form class="card-reservation__form">
      ...
      <div class="card-reservation__form-item" v-if="courses.length">
        <select class="card-reservation__select" @change="courseChanged">
          <option value="" :selected="!selectedCourseIndex">コースを選択してください (任意)</option>
          <option
    ...
    <div class="card-reservation__confirmation">
      ...
      <p class="card-reservation__confirmation-item" v-if="selectedCourseIndex !== undefined">Course<span>{{courses[selectedCourseIndex].name}}</span></p>
      <p class="card-reservation__confirmation-item" v-if="number && selectedCourseIndex !== undefined">Total<span>¥{{courses[selectedCourseIndex].price * +number.slice(0, -1)}}</span></p>
    </div>
...
export default Vue.extend({
  ...
  methods: {
    dateChanged(e: Event): void {
      ...
      this.$emit('dateChanged', target.value);
    },
    courseChanged(e: Event): void {
      const target = e.target as HTMLInputElement;
      if (target.value === '') {
        this.$emit('courseChanged', undefined);
      } else {
        this.$emit('courseChanged', +target.value);
      }
    },
    buttonClicked(): void {
